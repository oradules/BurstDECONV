...
def fit3_M1(kminmin,Kstoremin,shiftmin,alphamin,censmin,sd):
    ...
    # NS = len(xsgmin) 
    # NL = len(xlmin)
    if np.size(indobjneg)==0:
        reslM1= np.array([np.min(K1[:]),np.min(K5[:]),np.min(K2[:]),np.min(K4[:]),np.min(K3[:]),np.min(l1[:]),np.min(l2[:]),np.min(l3[:]),np.min(A1[:]),np.min(A2[:]),np.min(A3[:])])
        reshM1= np.array([np.max(K1[:]),np.max(K5[:]),np.max(K2[:]),np.max(K4[:]),np.max(K3[:]),np.max(l1[:]),np.max(l2[:]),np.max(l3[:]),np.max(A1[:]),np.max(A2[:]),np.max(A3[:])])
    else:
