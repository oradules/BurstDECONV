...
def predictPositions(cellnumber, DataExp, generations, fParam, colision):

    if colision:
        from optimize_local_col import optimize_local_col
    else:
    ...
    DataExpSmooth= np.nan_to_num(DataExp[:,iexp]) # taking the data cell by cell
    ...
    if colision:
        result_local = optimize_local_col(DataExpSmooth,x_GA_art,num_possible_poly,FreqEchSimu, FreqEchImg, TaillePreMarq,
