...
class readDataInFile:
    ...
    def read_data(self,data_i):
        ...
        DataExp=np.delete(DataExp, list(range(2, DataExp.shape[1], 3)), axis=1)
        
        Spot_names=pd.read_excel(DataFileName,usecols=np.arange(3,len(DataExp[0])*3-2),skipfooter =len(DataExp)+1).columns.tolist()
        lst1 = list(range(1, len(Spot_names), 3)) 
        lst2 = list(range(2, len(Spot_names), 3))
        Spot_names = np.delete(Spot_names, lst1+lst2)
        Spot_names = list(itertools.chain.from_iterable((e+' 1st act', e+' 2nd act') for e in Spot_names ))
